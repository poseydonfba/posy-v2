...
(function ($) {
    ...
    $('html').on("mouseover", ".ms-cnt-col02", function () {
    ...
    /* Abre tooltips de menu */

    $(document).on("click", ".tooltip-dropmenu", function () {
        $(this).tipsy("show");
    });

    /* Tooltips maiores com informacoes do perfil
    Ex: 1 DIV com uma IMG dentro, ao passar o mouse primeiro nha div e depois na img, mouseenter(faz uma vez) mouseover(faz duas vezes)
    -------------------------------- */
    $(document).on('mouseenter', '.cmt-cnt-img-perfil-tooltip-big', function () {
        $('body').find('.tipsy').each(function () {
            $(this).remove();
        });
...
})(jQuery);var V_TIME_MESSAGE = 4000;
...
};var _txtEmail, _txtSenha, _ckPersistLogin, _btnLogin,
    _txtCadNome, _txtCadSobrenome, _txtCadEmail, _txtCadSenha, _txtCadRepetirSenha, _dropCadSexo, _dropCadEstadoCivil, _dropCadDNDia, _dropCadDNMes, _dropCadDNAno, _btnCadastrar, _txtCaptcha,
    _txtReturnUrl;

$(document).ready(function () {

    _txtReturnUrl = $("#txtReturnUrl");

    _txtEmail = $("#txtEmail");
    _txtSenha = $("#txtSenha");
    _ckPersistLogin = $("#ckPersistLogin");
    _btnLogin = $(".btnLogin");

    _txtCadNome = $("#txtCadNome");
    _txtCadSobrenome = $("#txtCadSobrenome");
    _txtCadEmail = $("#txtCadEmail");
    _txtCadSenha = $("#txtCadSenha");
    _txtCadRepetirSenha = $("#txtCadRepetirSenha");
    _dropCadSexo = $("#dropCadSexo");
    _dropCadEstadoCivil = $("#dropCadEstadoCivil");
    _dropCadDNDia = $("#dropCadDNDia");
    _dropCadDNMes = $("#dropCadDNMes");
    _dropCadDNAno = $("#dropCadDNAno");
    _txtCaptcha = $("#txtCaptcha");
    _btnCadastrar = $(".btnCadastrar");

    fnCarregarDropAno(_dropCadDNAno);
    fnCarregarDropMes(_dropCadDNMes);
    fnCarregarDropDia(_dropCadDNDia);

    _txtEmail.focus();

    _btnCadastrar.click(function () {
        ...
        objWsPub.metodo = "CaptchaValid";
        objWsPub.data = "{'p_captcha':'" + _txtCaptcha.val() + "'}";
        objWsPub.ajaxWs(function (objresult) {

            if (parseInt(objresult.valid) == 1) {

                var objeto = new Object();
                objeto.Nome = _txtCadNome.val();
                objeto.Sobrenome = _txtCadSobrenome.val();
                objeto.Pais = "BR";
                objeto.DtNascimento = _dropCadDNAno.val() + "-" + _dropCadDNMes.val() + "-" + _dropCadDNDia.val();
                objeto.Sexo = _dropCadSexo.val();
                objeto.EstadoCivil = _dropCadEstadoCivil.val();
                objeto.Email = _txtCadEmail.val();
                objeto.Senha = _txtCadSenha.val();

                var dto = { "objeto": objeto };

                objWsPub.metodo = "CadastrarPerfil";
                objWsPub.data = JSON.stringify(dto);
                objWsPub.ajaxWs(function (objresult) {

                    objContainerLoading.buttonLoader('stop');

                    if (parseInt(objresult.sucesso) == 1)
                        fnLocationHref(objeto.msg);
                    else
                        fnNotificacao("warning", objresult.msg, V_TIME_MESSAGE);
    ...
    _btnLogin.click(function () {
        ...
        var objeto = new Object();
        objeto.Email = _txtEmail.val();
        objeto.Senha = _txtSenha.val();
        objeto.Persistir = _ckPersistLogin.is(":checked");

        var dto = { "objeto": objeto };

        objWsPub.metodo = "Login";
        objWsPub.data = JSON.stringify(dto);
        objWsPub.ajaxWs(function (objresult) {

            objContainerLoading.buttonLoader('stop');

            if (objresult.sucesso == 1) {
                var url = (_txtReturnUrl.val() === undefined) ? "inicio" : _txtReturnUrl.val();
                fnLocationHref(url);
            } else {
                fnNotificacao("warning", objresult.msg, V_TIME_MESSAGE);
            }
