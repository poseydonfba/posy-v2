...
namespace Posy.V2.Service
{
    public class TopicoService : ITopicoService
    {
        ...
        /// <returns></returns>
        public Topico SalvarTopico(Guid comunidadeId, string titulo, string descricao, TipoTopico fixo)
        {
        ...
        public void ExcluirTopico(Guid topicoId)
        {
            ...
            var moderador = _moderadorService.Obter(comunidade.ComunidadeId, _currentUser.GetCurrentUserId());
            if (moderador != null)
            {
                Excluir(topicoId);
                return;
        ...
        private void Excluir(Guid topicoId)
        {
        ...
        /// </summary>
        /// <param name="topicoId"></param>
        public void ExcluirTopicoPermanente(Guid topicoId)
        {
            var t = Obter(topicoId);
            if (t == null)
                throw new Exception("Erro");
        ...
        private void ExcluirPermanente(Guid topicoId)
        {
            var topico = Obter(topicoId);
        ...
        public IEnumerable<Topico> ObterTopicos(Guid comunidadeId, int paginaAtual, int itensPagina, out int totalRecords)
        {
            ...
            var membro = _membroService.Obter(comunidade.ComunidadeId, _currentUser.GetCurrentUserId());
            if (membro != null)
                return _repository.GetTopicos(comunidadeId, paginaAtual, itensPagina, out totalRecords);
