...
namespace Posy.V2.Infra.Data.Repository
{
    public class ConnectionRepository : Repository<Connection, string>, IConnectionRepository
    {
        ...
        public IEnumerable<Connection> ObterAmigosConectados(int usuarioId)
        {
            ...
            var amigosSolicitados = (from t in _db.Session.Query<Amizade>()/*.AsNoTracking()*/
                                     where t.SolicitadoPorId == usuarioId
                                     && t.StatusSolicitacao == SolicitacaoAmizade.Aprovado
                                     && t.Der == null
                                     select t.SolicitadoPara.Perfil);

            var amigosSolicitantes = (from t in _db.Session.Query<Amizade>()/*.AsNoTracking()*/
            ...
            var amigosConectados = DbSet/*.AsNoTracking()*/.Fetch(x => x.Usuario.Perfil).Where(x => amigos.Any(y => y.Id == x.UsuarioId) && x.Connected == true);
