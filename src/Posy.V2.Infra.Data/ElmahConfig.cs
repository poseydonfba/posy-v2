...
namespace Posy.V2.Infra.Data
{
    public static class ElmahConfig
    {
        public static string Up()
        {
            return @"CREATE TABLE [dbo].[ELMAH_Error]
                     (
                    ...
                    EXEC('CREATE NONCLUSTERED INDEX [IX_ELMAH_Error_App_Time_Seq] ON [dbo].[ELMAH_Error] 
                    (
                    [Application] ASC,
                    [TimeUtc] DESC,
                    [Sequence] DESC
        ...
        {
            return @"EXEC('DROP PROCEDURE [ELMAH_GetErrorXml]')
                    EXEC('DROP PROCEDURE [ELMAH_GetErrorsXml]')
                    EXEC('DROP PROCEDURE [ELMAH_LogError]')
