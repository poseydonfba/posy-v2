...
namespace Posy.V2.MVC.Hub
{
    ...
    public class ChatService : IChatService
    {
        ...
        public void OnConnected(Guid userId, string connectionId, string userAgent, IHubCallerConnectionContext<dynamic> clients)
        {
            #region V1
        ...
        private void SaveUserConnection(Guid userId, string connectionId, string userAgent)
        {
            try
            {
                _connectionService.ExcluirOldConnections(userId);
        ...
        public void OnDisconnected(Guid userId, string connectionId, IHubCallerConnectionContext<dynamic> clients)
        {
            try
            {
        ...
        public void Broadcast(string connectionId, string message, string username, IHubCallerConnectionContext<dynamic> clients)
        {
            ...
            clients.Client(connectionId).addChatMessageRecebida(username, message, time); // mostra a mensagem para o destino
