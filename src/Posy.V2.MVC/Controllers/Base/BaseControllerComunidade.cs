...
namespace Posy.V2.MVC.Controllers
{
    public class BaseControllerComunidade : BaseController
    {
        ...
        protected override void Initialize(RequestContext requestContext)
        {
            ...
            PerfilLogged = _perfilService.Obter(Guid.Parse(requestContext.HttpContext.User.Identity.GetUserId()));
            ...
            if (routeData.Values.ContainsKey("MS_DirectRouteMatches"))
            {
                ...
                {
                    if (!Guid.TryParse(MS_DirectRouteMatches.Values["topicoid"].ToString(), out Guid topicoId))
                        topicoId = Guid.Empty;
        ...
        {
            if (Guid.TryParse(idview, out Guid id))
