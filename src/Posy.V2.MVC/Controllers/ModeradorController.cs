...
namespace Posy.V2.MVC.Controllers
{
    ...
    public class ModeradorController : BaseControllerComunidade
    {
        ...
        public ActionResult AddCmmModerador(AdicionarModeradorModel model)
        {
            using (_uow)
            {
                _moderadorService.AdicionarModerador(ComunidadeView.ComunidadeId, model.UsuarioId);
                _uow.Commit();
            }
            return Json("");
        }
        ...
        public ActionResult ExcluirModerador(ExcluirModeradorModel model)
        {
            using (_uow)
            {
                _moderadorService.ExcluirModerador(ComunidadeView.ComunidadeId, model.ModeradorId);
                _uow.Commit();
            }
            return Json("");
        }
        ...
        public ActionResult GetCmmModeradores()
        {
            ...
            if (validacao.IsDono || validacao.IsModerador || validacao.IsMembro || validacao.IsPublica)
            {
                ...
                int totalRecords;
                var moderadores = _moderadorService.ObterModeradores(ComunidadeView.ComunidadeId, PAGE_NUMBER, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
                ...
                if (moderadores.Count > 0)
                {
                    ...
                    foreach (var moderador in moderadores)
                    {
                        ...
                        urlResolve = Funcao.ResolveServerUrl("~/Images/perfil/" + perfilModerador.UsuarioId.ToString() + "/1.jpg", false);
                        ...
                        if (validacao.IsDono || (validacao.IsModerador && perfilModerador.UsuarioId == PerfilLogged.UsuarioId))
                        {
