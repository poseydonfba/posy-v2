...
namespace Posy.V2.MVC.Controllers
{
    ...
    public class ChatController : BaseControllerPerfil
    {
        ...
        public ActionResult GetChatMembers()
        {
            ...
            int totalRecords;
            var amigos = _amizadeService.ObterAmigos(PerfilLogged.UsuarioId, 1, FuncaoSite.TOTAL_ITEM_BLOCO * 1000000, out totalRecords).ToList();

            foreach (var amigo in amigos)
            {
                var pathImg = Server.MapPath("~/Images/perfil/" + amigo.Perfil.UsuarioId.ToString() + "/1.jpg");
