...
namespace Posy.V2.MVC.Controllers
{
    ...
    public class MembroController : BaseControllerComunidade
    {
        ...
        public ActionResult ExcluirMembro(AceitarMembroModel model)
        {
            using (_uow)
            {
                _membroService.ExcluirMembro(ComunidadeView.ComunidadeId, model.MembroId);
                _uow.Commit();
            }
            return Json("");
        }
        ...
        public ActionResult AceitarMembro(AceitarMembroModel model)
        {
            using (_uow)
            {
                _membroService.AceitarMembro(ComunidadeView.ComunidadeId, model.MembroId);
                _uow.Commit();
            }
            return Json("");
        }
        ...
        public ActionResult RecusarMembro(RecusarMembroModel model)
        {
            using (_uow)
            {
                _membroService.RecusarMembro(ComunidadeView.ComunidadeId, model.MembroId);
                _uow.Commit();
            }
            return Json("");
        }
        ...
        public ActionResult getCmmMembros()
        {
            ...
            if (validacao.IsDono || validacao.IsModerador || validacao.IsMembro || validacao.IsPublica)
            {
                ...
                int totalRecords;
                var membros = _membroService.ObterMembros(ComunidadeView.ComunidadeId, PAGE_NUMBER, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
                ...
                if (membros.Count > 0)
                {
                    ...
                    foreach (var membro in membros)
                    {
                        ...
                        var validacao1 = (validacao.IsDono && ComunidadeView.UsuarioId != perfilMembro.UsuarioId);
                        // Se eu sou moderador da cmm e o membro currente não é o dono (Pode excluir)
                        var validacao2 = (validacao.IsModerador && ComunidadeView.UsuarioId != perfilMembro.UsuarioId);
                        // Se eu sou membro da cmm e o membro currente não é o meu (Pode excluir) 
                        var validacao3 = (validacao.IsMembro && perfilMembro.UsuarioId == PerfilLogged.UsuarioId);
        ...
        public ActionResult getCmmMembrosPendente()
        {
            ...
            if (validacao.IsDono || validacao.IsModerador)
            {
                ...
                int totalRecords;
                var membrosPendentes = _membroService.ObterMembrosPendentes(ComunidadeView.ComunidadeId, PAGE_NUMBER, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
                ...
                if (membrosPendentes.Count > 0)
                {
                    ...
                    foreach (var membroPendente in membrosPendentes)
                    {
                        ...
                        urlResolve = Funcao.ResolveServerUrl("~/Images/perfil/" + perfilPendente.UsuarioId.ToString() + "/1.jpg", false);
