...
namespace Posy.V2.MVC.Controllers
{
    ...
    public class ForumController : BaseControllerComunidade
    {
        ...
        public ActionResult SalvarTopico(SalvarTopicoModel model)
        {
            using (_uow)
            {
                var topico = _topicoService.SalvarTopico(ComunidadeView.ComunidadeId, model.Titulo, model.Descricao, model.Fixo);
                _uow.Commit();
                ...
                urlEncryptadaCmm = FuncaoSite.getUrlNomeIdCmm(cmm.Alias) + "/" + FuncaoSite.ROUTE_URL_TOPICO + "/" + topico.TopicoId.ToString();
                urlResolve = Funcao.ResolveServerUrl("~/Images/perfil/" + perfilDonoTpc.UsuarioId.ToString() + "/1.jpg", false);
        ...
        public ActionResult SalvarTopicoPost(SalvarTopicoPostModel model)
        {
            using (_uow)
            {
                ...
                {
                    v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' codpost='" + post.TopicoPostId.ToString() + "' opcao='E'>"+ UIConfig.ExcluirPostagem +@"</div></a>";
                }
        ...
        public ActionResult getCmmTopicos()
        {
            ...
            {
                int totalRecords;
                var topicos = _topicoService.ObterTopicos(ComunidadeView.ComunidadeId, PAGE_NUMBER, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
                ...
                foreach (var topico in topicos)
                {
                    ...
                    urlEncryptadaPerfil = FuncaoSite.getUrlNomeIdPerfil(perfilDonoTpc.Alias);
                    urlEncryptadaCmm = FuncaoSite.getUrlNomeIdCmm(ComunidadeView.Alias) + "/" + FuncaoSite.ROUTE_URL_TOPICO + "/" + topico.TopicoId + "/" + ultimaPagina;
                    ...
                    urlEncryptadaCmm = FuncaoSite.getUrlNomeIdCmm(ComunidadeView.Alias) + "/" + FuncaoSite.ROUTE_URL_TOPICO + "/" + topico.TopicoId.ToString();
        ...
        public ActionResult getCmmTopicosPost()
        {
            ...
            if (validacao.IsDono || validacao.IsModerador || validacao.IsMembro || validacao.IsPublica)
            {
                var moderadores = _moderadorService.ObterModeradores(ComunidadeView.ComunidadeId).ToList();
                ...
                foreach (var post in posts)
                {
                    ...
                    if (validacao.IsDono)
                    {
                        ...
                        if (perfilDonoTpc.UsuarioId != PerfilLogged.UsuarioId && perfilDonoTpcModerador == null)
                        {
                            v_html_menuitens += "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' codpost='" + post.TopicoPostId.ToString() + "' cc='" + ComunidadeView.ComunidadeId.ToString() + "' cp='" + perfilDonoTpc.UsuarioId.ToString() + "' opcao='M'>Add como moderador</div></a>";
                        }
                    }
                    else if (validacao.IsModerador)
                    {
                    ...
                    else if (perfilDonoTpc.UsuarioId == PerfilLogged.UsuarioId)
                    {
                        v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' codpost='" + post.TopicoPostId.ToString() + "' opcao='E'>" + UIConfig.ExcluirPostagem + @"</div></a>";
                    }
                    ...
                    if (perfilDonoTpc.UsuarioId == ComunidadeView.UsuarioId)
                    {
                        autorizacaoComu = "<span class='cmm-dono'>"+UIConfig.DonoDaComunidade+"</span>";
                    }
