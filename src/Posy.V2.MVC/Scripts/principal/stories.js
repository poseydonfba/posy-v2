    ...
    $.get("/ajax/stories", function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < perfil.stories.length; j++) {
                ...
                items.push(Zuck.buildItem(perfil.usuarioId + "-" + (j + 1), item.type, item.length,
                    "/Scripts/plugin/stories/img/stories/" + item.src,
                    "/Scripts/plugin/stories/img/stories/" + item.preview, item.link, item.linkText, item.seen, item.time));
            }
