...
namespace Posy.V2.MVC.Controllers
{
    ...
    public class DepoimentoController : BaseControllerPerfil
    {
        ...
        public ActionResult SalvarDepoimento(SalvarDepoimentoModel model)
        {
            using (_uow)
            {
                var depoimento = _depoimentoService.EnviarDepoimento(PerfilView.Id, model.Depoimento);
                _uow.Commit();
                ...
                urlResolve = Funcao.ResolveServerUrl("~/img/perfil/" + perfilDepoimento.Id.ToString() + "/1.jpg", false);

                v_html += @"
                    <div class='tl-cnt-default-block postado-recente' cd='" + depoimento.Id.ToString() + @"'>
                ...
                v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' cd='" + depoimento.Id.ToString() + "' opcao='E'>Excluir</div></a>";

                v_html += @"
                    <div class='popr tooltip-dropmenu-container' cd='" + depoimento.Id.ToString() + @"'>
                        <div class='button-group minor-group'>

                        <div class='tooltip-dropmenu'>
                            <a href='javascript:void(0);' class='button icon settings semtexto' title=''></a>
                            <div class='tooltip-dropmenu-html-container' cd='" + depoimento.Id.ToString() + @"'>
                                <div class='tooltip-dropmenu-menu' cd='" + depoimento.Id.ToString() + @"'>
                                    " + v_html_menuitens + @"
                                </div>
                            </div>
                            <div class='clear'></div>
        ...
        public ActionResult GetListDepoimentos()
        {
            ...
            {
                int totalRecords;
                var depoimentos = _depoimentoService.ObterDepoimentosRecebidos(PerfilView.Id, PAGE_NUMBER, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
                ...
                if (depoimentos.Count > 0)
                {
                    foreach (var depoimento in depoimentos)
                    {
                        ...
                        urlResolve = Funcao.ResolveServerUrl("~/Images/perfil/" + usuarioDepoimento.Id.ToString() + "/1.jpg", false);

                        v_html += @"
                                <div class='tl-cnt-default-block' cd='" + depoimento.Id.ToString() + @"'>
                                    <div class='tl-cnt-default-img cd-picture'>
                                        <img src='" + urlResolve + @"' alt='Picture' />
                                    </div> <!-- tl-cnt-default-img -->
                        ...
                        {
                            if (usuarioDepoimento.Id != PerfilLogged.Id)
                            {
                                ...
                                {
                                    v_html_menuitens = @"
                                            <a href='javascript:void(0);'><div class='menuitem btnMenuItem' cd='" + depoimento.Id.ToString() + @"' opcao='A'>"+ UIConfig.aceitar +@"</div></a>
                                            <a href='javascript:void(0);'><div class='menuitem btnMenuItem' cd='" + depoimento.Id.ToString() + @"' opcao='R'>" + UIConfig.recusar + @"</div></a>
                                        ";
                                ...
                                {
                                    v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' cd='" + depoimento.Id.ToString() + "' opcao='E'>" + UIConfig.excluir + @"</div></a>";
