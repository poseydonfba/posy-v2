    ...
    public class VideoController : BaseControllerPerfil
    {
        ...
        public ActionResult SalvarVideo(SalvarVideoModel model)
        {
            using (_uow)
            {
                ...
                v_html += "<div class='video-container' codvideo='" + video.Id.ToString() + "'>";

                #region TOOLTIP MENU DE OPCOES

                v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' codvideo='" + video.Id.ToString() + "' opcao='VE'>Excluir vídeo</div></a>";

                v_html += @"
                    <div class='popr tooltip-dropmenu-container' codvideo='" + video.Id.ToString() + @"'>
                        <div class='button-group minor-group'>

                        <div class='tooltip-dropmenu'> 
                            <a href='javascript:void(0);' class='button icon settings semtexto' title=''></a>
                            <div class='tooltip-dropmenu-html-container' codvideo='" + video.Id.ToString() + @"'>
                                <div class='tooltip-dropmenu-menu' codvideo='" + video.Id.ToString() + @"'>
                                    " + v_html_menuitens + @"
                                </div>
                            </div>
                            <div class='clear'></div>
                ...
                v_html += @"
                    <a href='javascript:void(0);' class='image-foto'>
                        <img class='gl-zoom-img' src='http://img.youtube.com/vi/" + video.Url + "/0.jpg' codvideo='" + video.Id.ToString() + "' videoid='" + video.Url + "' videotitle='" + video.NomeHtml + @"' />
                    </a>
        ...
        public ActionResult SalvarVideoComentario(SalvarVideoComentarioModel model)
        {
            using (_uow)
            {
                ...
                v_html += @"
                    <div class='cmt-cnt postado-recente' codcoment='" + comentario.Id.ToString() + @"'>
        ...
        public ActionResult GetListVideos()
        {
            ...
            {
                int totalRecords;
                var videos = _videoService.ObterVideos(PerfilView.Id, PAGE_NUMBER, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
                ...
                if (videos.Count > 0)
                {
                    ...
                    foreach (var video in videos)
                    {
                        ...
                        {
                            v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' codvideo='" + video.Id.ToString() + "' opcao='EV'>Excluir vídeo</div></a>";
                        ...
                        var comentarios = _videoComentarioService.ObterComentarios(video.Id, PAGE_NUMBER, FuncaoSite.TOTAL_COMENT_PAGE).ToList();
                        ...
                        foreach (var comentario in comentarios)
                        {
                            ...
                            else if (validacao.IsMeuPerfil())
                            {
                                v_html_menuitens = @"
                                    ...
                                    <a href='javascript:void(0);'><div class='menuitem btnMenuItem' codcoment='" + comentario.Id.ToString() + "' codperfil='" + usuarioComentario.Id.ToString() + @"' opcao='B'>" + UIConfig.BloquearPostagensDessePerfil + @"</div></a>
                                ";
                            }
                        ...
                        var proximosComentarios = _videoComentarioService.ObterComentarios(video.Id, nextPage, FuncaoSite.TOTAL_COMENT_PAGE).ToList();
                        if (proximosComentarios.Count > 0)
                        {
                            v_html += @"
                                    <div class='ed-cnt-cmt-more button-group minor-group'>
                                         <a href='javascript:void(0);' class='button minor1 btnVerMais' codvideo='" + video.Id.ToString() + "' page='" + nextPage.ToString() + "'>" + BTN_VER_MAIS_COMENTS + @"</a>
                                    </div>
                                ";
                        }
        ...
        public ActionResult GetListVideoComentarios(int videoid, int page)
        {
            var v_html = string.Empty;
            var v_html_menuitens = string.Empty;
