...
namespace Posy.V2.MVC.Controllers
{
    ...
    public class RecadoController : BaseControllerPerfil
    {
        ...
        public ActionResult SalvarRecado(SalvarRecadoModel model)
        {
            ...
            using (_uow)
            {
                recado = _recadoService.EnviarRecado(PerfilView.Id, model.RecadoHtml);
                _uow.Commit();

                var v_codrecado = recado.Id.ToString();
                ...
                v_html = @"
                        <div class='tl-cnt-default-block postado-recente' codrecado='" + recado.Id.ToString() + "' codperfil='" + PerfilLogged.Id.ToString() + @"'>
                ...
                if (PerfilView.Id == PerfilLogged.Id)
                {
                ...
                else
                {
                    v_html_menuitens = @"
                        ...
                        <a href='javascript:void(0);'><div class='menuitem btnMenuItem' codperfil='" + PerfilLogged.Id.ToString() + @"' opcao='RB'>" + UIConfig.BloquearPostagensDessePerfil + @"</div></a>
                    ";
                }
                ...
                urlResolve = Funcao.ResolveServerUrl("~/Images/perfil/" + perfilRecebeuRecado.Id.ToString() + "/1.jpg", false);
                ...
                var usuarioDoRecadoBloqueadoParaPostarNoUsuarioPost =
                    _perfilBloqueadoService.ObterPerfilBloqueado(perfilRecebeuRecado.Id, PerfilLogged.Id);
                ...
                if (1 == 2)
                {
                    ...
                    //    if (usuarioComentario.UsuarioId == PerfilLogged.Id)
                    //    {
        ...
        public ActionResult SalvarRecadoComentario(SalvarRecadoComentarioModel model)
        {
            using (_uow)
            {
                ...
                var usuarioComPostBloqueado = _perfilBloqueadoService.ObterPerfilBloqueado(recado.EnviadoPorId, PerfilLogged.Id);
                ...
                v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' codcoment='" + comentario.Id.ToString() + "' opcao='RCE'>" + UIConfig.ExcluirComentário + @"</div></a>";
        ...
        public ActionResult GetListRecados()
        {
            ...
            using (_uow)
            {
                ...
                {
                    var recados = _recadoService.ObterRecadosEnviadosERecebidos(PerfilView.Id, PAGE_NUMBER, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
                    ...
                    if (recados.Count > 0)
                    {
                        ...
                        foreach (var recado in recados)
                        {
                            ...
                            var perfilBloqueado = _perfilBloqueadoService.ObterPerfilBloqueado(usuarioRecebeuRecado.Id, usuarioRecado.Id);

                            #region CONTAINER

                            urlResolve = Funcao.ResolveServerUrl("~/Images/perfil/" + usuarioRecado.Id.ToString() + "/1.jpg", false);

                            #region HTML

                            v_html += @"
                                        <div class='tl-cnt-default-block' codrecado='" + recado.Id.ToString() + @"' style='display:none;'>
                                            <div class='tl-cnt-default-img cd-picture'>
                                                    <img src='" + urlResolve + @"' alt='Picture' />
                                            </div> <!-- tl-cnt-default-img -->
                            ...
                            if (usuarioRecado.Id == PerfilLogged.Id)
                            {
                                v_html_menuitens = "<a href='javascript:void(0);'><div class='menuitem btnMenuItem' codrecado='" + recado.Id.ToString() + @"' opcao='RE'>" + UIConfig.Excluirrecado + @"</div></a>";
                            }
                            else if (validacao.IsMeuPerfil())
                            {
                                v_html_menuitens = @"
                                            <a href='javascript:void(0);'><div class='menuitem btnMenuItem' codrecado='" + recado.Id.ToString() + @"' opcao='RE'>" + UIConfig.Excluirrecado + @"</div></a>
                                        ";

                                if (perfilBloqueado == null)
                                    v_html_menuitens += @"
                                            <a href='javascript:void(0);'><div class='menuitem btnMenuItem' codrecado='" + recado.Id.ToString() + @"' codperfil='" + usuarioRecado.Id.ToString() + @"' opcao='RB'>" + UIConfig.BloquearPostagensDessePerfil + @"</div></a>
                                        ";
                            }
                            ...
                            {
                                v_html += @"
                                            <div class='popr tooltip-dropmenu-container' codrecado='" + recado.Id.ToString() + @"'>
                                                <div class='button-group minor-group'>

                                                <div class='tooltip-dropmenu'> 
                                                    <a href='javascript:void(0);' class='button icon settings semtexto' title=''></a>
                                                    <div class='tooltip-dropmenu-html-container' codrecado='" + recado.Id.ToString() + @"'>
                                                        <div class='tooltip-dropmenu-menu' codrecado='" + recado.Id.ToString() + @"'>
                                                            " + v_html_menuitens + @"
                                                        </div>
                                                    </div>
                                                    <div class='clear'></div>
                            ...
                            if (validacao.IsMeuPerfil())
                                v_html += "<input type='checkbox' class='tl-checkbox' value='" + recado.Id.ToString() + "' />";

                            urlEncryptadaPerfil = FuncaoSite.getUrlNomeIdPerfil(usuarioRecado.Perfil.Alias);
                            urlResolve = Funcao.ResolveServerUrl("~/Images/perfil/" + usuarioRecebeuRecado.Id.ToString() + "/1.jpg", false);
                            ...
                            if (usuarioRecado.Id == PerfilLogged.Id ||
                                usuarioRecebeuRecado.Id == PerfilLogged.Id)
                            {

                                #region CONTAINER EDITOR

                                var usuarioDoRecadoBloqueadoParaPostarNoUsuarioPost =
                                    _perfilBloqueadoService.ObterPerfilBloqueado(usuarioRecado.Id, PerfilLogged.Id);
                                ...
                                var comentarios = _recadoComentarioService.ObterComentarios(recado.Id, 1, FuncaoSite.TOTAL_COMENT_PAGE);
                                ...
                                foreach (var comentario in comentarios)
                                {
                                    ...
                                    v_html += @"
                                                <div class='cmt-cnt' codcoment='" + comentario.Id.ToString() + @"'>
                                                    <img class='cmt-cnt-img-perfil cmt-cnt-img-perfil-tooltip-big' src='" + urlResolve + @"' />
                                            ";
                                    ...
                                    {
                                        v_html_menuitens = @"
                                                    <a href='javascript:void(0);'><div class='menuitem btnMenuItem' codcoment='" + comentario.Id.ToString() + @"' opcao='RCE'>" + UIConfig.ExcluirComentário + @"</div></a>
                                                    <a href='javascript:void(0);'><div class='menuitem btnMenuItem' codperfil='" + usuarioComentario.Id.ToString() + @"' opcao='RCB'>" + UIConfig.BloquearPostagensDessePerfil + @"</div></a>
                                                ";
                                    }
                                ...
                                var proximosComentarios = _recadoComentarioService.ObterComentarios(recado.Id, nextPage, FuncaoSite.TOTAL_COMENT_PAGE).ToList();
                                if (proximosComentarios.Count > 0)
                                {
                                    v_html += @"
                                                <div class='ed-cnt-cmt-more button-group minor-group'>
                                                     <a href='javascript:void(0);' class='button minor1 btnVerMais' codrecado='" + recado.Id.ToString() + @"' page='" + nextPage.ToString() + @"'>" + BTN_VER_MAIS_COMENTS + @"</a>
                                                </div>
                                            ";
                                }
                            ...
                            if (usuarioRecebeuRecado.Id == PerfilLogged.Id)
                                _recadoService.MarcarComoLido(recado.Id);
                        }
                        ...
                        var proximosPosts = _recadoService.ObterRecadosEnviadosERecebidos(PerfilView.Id, nextPage, FuncaoSite.TOTAL_POST_PAGE, out totalRecords).ToList();
        ...
        public ActionResult GetListRecadoComentarios(int recadoid, int page)
        {
            var v_html = string.Empty;
            var v_html_menuitens = string.Empty;
